...
import { FOUNDER_ROLE } from '../src/authRoles';

async function main() {
  const db = new PostgresClient();
  await db.query(`INSERT INTO users (id, name, role, auth_provider, is_demo) VALUES ('admin-1', 'Founder', '${FOUNDER_ROLE}', 'bootstrap', TRUE) ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, role = EXCLUDED.role, updated_at = now()`);
