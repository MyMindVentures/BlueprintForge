    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-black/90 border border-white/10 rounded-2xl p-6 max-w-sm w-full shadow-2xl relative overflow-hidden">
        ...
        <div className="flex items-center gap-3 mb-4 text-accent">
          ...
          <h3 className="text-xl font-bold text-white tracking-tight">{tx("uiLegacy.components.layout.newversionpopup.001")}</h3>
        </div>
        
        <div className="mb-6 space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-sm font-black text-white/50 tracking-wider uppercase">{tx("uiLegacy.components.layout.newversionpopup.002")}</span>
            <span className="px-2 py-0.5 bg-accent/20 text-accent text-xs font-mono rounded font-bold">{latestVersion.version}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-sm font-black text-white/50 tracking-wider uppercase">{tx("uiLegacy.components.layout.newversionpopup.003")}</span>
            <span className="text-sm text-white/80">
              {latestVersion.created_at ? new Date(latestVersion.created_at).toLocaleDateString() :tx("uiStrings.components.layout.newversionpopup.001")}
            </span>
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          >{tx("uiLegacy.components.layout.newversionpopup.004")}</button>
          <button 
          ...
          >{tx("uiLegacy.components.layout.newversionpopup.005")}<ArrowRight size={16} />
          </button>
        </div>
      </div>
    </div>
