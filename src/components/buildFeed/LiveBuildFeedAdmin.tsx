          ...
          <div className="flex gap-4">
             <div className="bg-white/[0.02] border border-white/5 rounded-2xl px-6 py-3 flex items-center gap-4">
                <div className="text-right">
                  <p className="text-[10px] font-black text-text-dim uppercase tracking-widest">{t("auto.liveBuildFeedAdmin.activeFocus69e335")}</p>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1.2fr_1fr] gap-12 items-start">
          ...
          <div className="space-y-8">
            <div className="space-y-6">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-sm font-black text-white uppercase tracking-[0.2em]">{t("auto.liveBuildFeedAdmin.publishNewRequeste5911f")}</h2>
              </div>
              
              <div className="bg-[#111] border border-white/10 rounded-[32px] p-6 md:p-8 space-y-6 shadow-2xl">
                <label className="block space-y-2">
                  <span className="text-[10px] font-black text-text-dim uppercase tracking-[0.2em] ml-2">{t("auto.liveBuildFeedAdmin.rawFounderInputb2c9e2")}</span>
                  <textarea
                    value={rawInput}
                    onChange={(e) => setRawInput(e.target.value)}
                    placeholder={t("bootstrap.placeholder")}
                ...
                <div className="bg-accent/5 border border-accent/20 rounded-[32px] p-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 shadow-2xl shadow-accent/5">
                  <div className="flex items-center justify-between gap-4 border-b border-accent/10 pb-6">
                    <div className="flex-1">
                       <p className="text-[10px] font-black text-accent uppercase tracking-[0.2em] mb-2">{t("auto.liveBuildFeedAdmin.specTitle28eab7")}</p>
                       {isEditing ? (
                         <input
                           type="text"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-6">
                       <label className="block space-y-2">
                         <span className="text-[10px] font-black text-text-dim uppercase tracking-[0.2em]">{t("auto.liveBuildFeedAdmin.contexte77109")}</span>
                         <textarea 
                           readOnly={!isEditing}
                    ...
                    <div className="space-y-6">
                       <label className="block space-y-2">
                         <span className="text-[10px] font-black text-text-dim uppercase tracking-[0.2em]">{t("auto.liveBuildFeedAdmin.requestedChange3d73c4")}</span>
                         <textarea 
                           readOnly={!isEditing}
            ...
            <div className="space-y-6 pt-12">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-sm font-black text-white uppercase tracking-[0.2em]">{t("auto.liveBuildFeedAdmin.activeBuildStream27752c")}</h2>
                </div>
              </div>
          ...
          <aside className="space-y-12">
             <div className="space-y-6">
                <div className="flex items-center gap-3">
                  <Radio size={20} className="text-accent" />
                  <h2 className="text-sm font-black text-white uppercase tracking-[0.2em]">{t("auto.liveBuildFeedAdmin.dailySignalc5a68b")}</h2>
                </div>
                
                <div className="bg-[#111] border border-white/10 rounded-[32px] p-8 space-y-6 shadow-2xl">
                   <p className="text-[10px] font-black text-text-dim uppercase tracking-[0.2em]">{t("auto.liveBuildFeedAdmin.broadcastToAllBuildersdb8c74")}</p>
                   <textarea
                     value={signalText}
                     onChange={(e) => setSignalText(e.target.value)}
                     placeholder={t("buildFeed.dailySignalPlaceholder")}
                ...
                <div className="space-y-4 pt-6">
                  <p className="text-[10px] font-black text-text-dim uppercase tracking-[0.2em] ml-4">{t("auto.liveBuildFeedAdmin.signalHistory614d55")}</p>
