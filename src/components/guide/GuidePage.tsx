      ...
      <div className="max-w-6xl mx-auto px-6 py-12 md:py-24 space-y-32">
        ...
        <section className="text-center space-y-12 py-12 relative overflow-hidden">
          ...
          <div className="relative z-10 space-y-6">
            <div className="inline-flex items-center gap-3 px-4 py-1.5 rounded-full bg-accent/10 border border-accent/20 text-accent text-[10px] font-black uppercase tracking-[0.3em] mx-auto">
              <HelpCircle size={14} fill="currentColor" />{tx("uiLegacy.components.guide.guidepage.001")}</div>
            <h1 className="text-5xl md:text-8xl font-black uppercase tracking-tight leading-[0.85]">{tx("uiLegacy.components.guide.guidepage.002")}</h1>
            <p className="text-lg md:text-xl text-text-dim max-w-2xl mx-auto font-medium">{tx("uiLegacy.components.guide.guidepage.003")}</p>
          </div>
          <div className="relative z-10 flex flex-wrap items-center justify-center gap-6 pt-4">
             <button 
              ...
              >
               <Play size={18} fill="currentColor" />{tx("uiLegacy.components.guide.guidepage.004")}</button>
             <button className="glass-btn-secondary !px-10 !py-4 border-white/5">{tx("uiLegacy.components.guide.guidepage.005")}{latestVersion?.version}
             </button>
             <button className="glass-btn-secondary !px-10 !py-4 border-white/5">{tx("uiLegacy.components.guide.guidepage.006")}</button>
          </div>
        </section>
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-black text-white uppercase tracking-widest">{tx("uiLegacy.components.guide.guidepage.007")}{latestVersion?.version}</h2>
              </div>
              <p className="text-3xl font-black text-white/90 uppercase tracking-tight">"{latestVersion?.release_title}"</p>
            </div>
            <div className="text-[10px] font-black text-text-dim uppercase tracking-widest bg-white/5 px-4 py-2 rounded-xl">{tx("uiLegacy.components.guide.guidepage.008")}{latestVersion ? formatDate(latestVersion.created_at, { dateStyle: 'medium' }) : tx('states.notAvailable')}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-12">
             <div className="space-y-10">
                <div className="space-y-4">
                  <h3 className="text-xs font-black text-accent uppercase tracking-widest flex items-center gap-2">
                    <Zap size={14} fill="currentColor" />{tx("uiLegacy.components.guide.guidepage.009")}</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                   <div className="space-y-4">
                      <h3 className="text-xs font-black text-text-dim uppercase tracking-widest flex items-center gap-2">{tx("uiLegacy.components.guide.guidepage.010")}</h3>
                   ...
                   <div className="space-y-4">
                      <h3 className="text-xs font-black text-text-dim uppercase tracking-widest flex items-center gap-2">{tx("uiLegacy.components.guide.guidepage.011")}</h3>
                      <ul className="space-y-2">
             ...
             <div className="space-y-6">
                <div className="bg-black/40 border border-white/5 p-6 rounded-3xl space-y-4">
                   <h4 className="text-[10px] font-black text-text-dim uppercase tracking-widest">{tx("uiLegacy.components.guide.guidepage.012")}</h4>
                ...
                <button className="w-full glass-btn-secondary !text-[10px] !py-3 !font-black uppercase tracking-[0.2em] border-white/5 flex items-center justify-center gap-2">
                   <Github size={14} />{tx("uiLegacy.components.guide.guidepage.013")}</button>
             </div>
          </div>
        </section>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
           <div className="space-y-12">
              <div className="space-y-4">
                <h2 className="text-[10px] font-black text-accent uppercase tracking-[0.6em]">{tx("uiLegacy.components.guide.guidepage.014")}</h2>
                <h3 className="text-4xl md:text-6xl font-black uppercase tracking-tight">{tx("uiLegacy.components.guide.guidepage.015")}<br />{tx("uiLegacy.components.guide.guidepage.016")}</h3>
              </div>
              <p className="text-xl text-text-dim leading-relaxed font-medium">{tx("uiLegacy.components.guide.guidepage.017")}</p>
              <div className="flex items-center gap-4 p-6 bg-accent/5 border-l-4 border-accent rounded-r-3xl">
                 ...
                 <p className="text-sm text-white/80 font-medium italic">{tx("uiLegacy.components.guide.guidepage.018")}</p>
              </div>
           </div>
           ...
           <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {coreCards.map((card, i) => (
                <div key={i} className="bg-white/[0.02] border border-white/5 p-6 rounded-3xl space-y-4 hover:border-accent/40 transition-all group">
                   ...
                   <button className="text-[10px] font-black text-accent uppercase tracking-widest flex items-center gap-2 group-hover:gap-3 transition-all pt-2">{tx("uiLegacy.components.guide.guidepage.019")}<ChevronRight size={12} />
                   </button>
                </div>
              ))}
           </div>
        ...
        <section className="space-y-12">
          <div className="text-center space-y-4">
            <h2 className="text-[10px] font-black text-accent uppercase tracking-[0.6em]">{tx("uiLegacy.components.guide.guidepage.020")}</h2>
            <h3 className="text-4xl md:text-6xl font-black uppercase tracking-tight">{tx("uiLegacy.components.guide.guidepage.021")}</h3>
            <p className="text-text-dim max-w-2xl mx-auto">{tx("uiLegacy.components.guide.guidepage.022")}</p>
          </div>
            ...
            {guideSections.map((section) => (
              <article key={section.id} id={section.id} className="rounded-[32px] border border-white/5 bg-white/[0.02] p-6 space-y-5">
                ...
                <div className="rounded-2xl border border-accent/10 bg-accent/[0.04] p-4">
                  <p className="text-[9px] font-black uppercase tracking-widest text-accent">{tx("uiLegacy.components.guide.guidepage.023")}</p>
        ...
        <section className="space-y-12">
          <div className="text-center space-y-4">
            <h2 className="text-[10px] font-black text-accent uppercase tracking-[0.6em]">{tx("uiLegacy.components.guide.guidepage.024")}</h2>
            <h3 className="text-4xl md:text-6xl font-black uppercase tracking-tight">{tx("uiLegacy.components.guide.guidepage.025")}</h3>
          </div>
            ...
            {Object.values(screenGuidance).map((screen) => (
              <div key={screen.title} className="rounded-[32px] border border-white/5 bg-[#0b0b0b] p-6 space-y-4">
                ...
                <div className="grid gap-3 text-xs text-white/70">
                  <p><span className="font-black uppercase tracking-widest text-white/35">{tx("uiLegacy.components.guide.guidepage.026")}</span> {screen.state}</p>
                  <p><span className="font-black uppercase tracking-widest text-white/35">{tx("uiLegacy.components.guide.guidepage.027")}</span> {screen.nextAction}</p>
                  <p><span className="font-black uppercase tracking-widest text-white/35">{tx("uiLegacy.components.guide.guidepage.028")}</span> {screen.disabledReason}</p>
                  <p><span className="font-black uppercase tracking-widest text-white/35">{tx("uiLegacy.components.guide.guidepage.029")}</span> {screen.empty}</p>
                  <p><span className="font-black uppercase tracking-widest text-white/35">{tx("uiLegacy.components.guide.guidepage.030")}</span> {screen.loading}</p>
                  <p><span className="font-black uppercase tracking-widest text-white/35">{tx("uiLegacy.components.guide.guidepage.031")}</span> {screen.error}</p>
                  <p><span className="font-black uppercase tracking-widest text-white/35">{tx("uiLegacy.components.guide.guidepage.032")}</span> {screen.success}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <section className="space-y-8">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            <div>
              <h2 className="text-[10px] font-black text-accent uppercase tracking-[0.6em]">{tx("uiLegacy.components.guide.guidepage.033")}</h2>
              <h3 className="mt-3 text-3xl md:text-5xl font-black uppercase tracking-tight">{tx("uiLegacy.components.guide.guidepage.034")}</h3>
            </div>
            <StatusBadge status="Published Version" label={tx("uiLegacy.components.guide.guidepage.035")} />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <section className="rounded-[48px] border border-white/5 bg-white/[0.02] p-8 md:p-12 space-y-8">
          <div>
            ...
            <h3 className="mt-3 text-3xl md:text-5xl font-black uppercase tracking-tight">{tx("uiLegacy.components.guide.guidepage.036")}</h3>
          </div>
          {[
        ...
        <section className="space-y-16">
          <div className="text-center space-y-4">
             <h2 className="text-[10px] font-black text-accent uppercase tracking-[0.6em]">{tx("uiLegacy.components.guide.guidepage.037")}</h2>
             <h3 className="text-4xl md:text-6xl font-black uppercase tracking-tight">{tx("uiLegacy.components.guide.guidepage.038")}</h3>
          </div>
                     ...
                     {flow.steps.map((step, i) => (
                       <div key={i} className="flex gap-6 items-start group/step">
                          ...
                          <div className="space-y-1 pt-0.5">
                             ...
                             <p className="text-[10px] text-text-dim italic">{tx("uiLegacy.components.guide.guidepage.039")}{step.expected_result}</p>
                          </div>
                       </div>
                     ))}
                  </div>
        ...
        {recordings.length > 0 && (
          <section className="space-y-12">
             <div className="flex items-center justify-between">
                <h3 className="text-2xl font-black uppercase tracking-widest">{tx("uiLegacy.components.guide.guidepage.040")}</h3>
                <div className="text-[10px] font-black text-text-dim uppercase tracking-widest">{recordings.length}{tx("uiLegacy.components.guide.guidepage.041")}</div>
             </div>
        ...
        <section className="text-center py-32 bg-accent/5 border border-accent/20 rounded-[80px] space-y-12 relative overflow-hidden">
          ...
          <div className="relative z-10 space-y-8 px-6">
            <h3 className="text-4xl md:text-6xl font-black uppercase tracking-tight text-white text-balance">{tx("uiLegacy.components.guide.guidepage.042")}<br />
              <span className="text-accent italic">{tx("uiLegacy.components.guide.guidepage.043")}</span>
            </h3>
            <div className="flex flex-wrap items-center justify-center gap-6 pt-6">
               <button className="glass-btn-primary !px-12 !py-5 shadow-2xl">{tx("uiLegacy.components.guide.guidepage.044")}</button>
               <button className="glass-btn-secondary !px-12 !py-5 border-white/10 hover:bg-white/5">{tx("uiLegacy.components.guide.guidepage.045")}</button>
            </div>
          </div>
        </section>
